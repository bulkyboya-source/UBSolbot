...
async def cmd_gems(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    placeholder = await update.message.reply_text("🔍 *Scanning for trending Solana memecoins...*", parse_mode="Markdown")
    ...
    try:
        ...
        try:
            ...
            if cg_resp.status_code == 200:
                trending_coins = cg_resp.json().get("coins", [])
                for coin in trending_coins:
                    name = coin.get("item", {}).get("name", "")
                    symbol = coin.get("item", {}).get("symbol", "")
                    if name:
                        trending_terms.append(name.lower())
                    if symbol:
                        trending_terms.append(symbol.lower())
        except Exception as e:
            logger.error(f"CoinGecko trending error: {e}")

        # Source 2: DexScreener latest Solana pairs
        try:
            resp = requests.get(
            ...
            if resp.status_code == 200:
                ...
                for profile in solana_profiles[:40]:
                    ...
                    lp_locked = pair.get("liquidity", {}).get("locked", False) or False
                    lp_burned = False
                    labels = pair.get("labels", []) or []
                    if "lp-locked" in labels or "lp-burned" in labels:
                        lp_burned = True
                    if not lp_locked and not lp_burned:
                        continue

                    # Check if from Pump.fun
                    is_pumpfun = False
                    if pair.get("dexId", "").lower() == "pump" or "pump.fun" in pair.get("url", "").lower():
                        is_pumpfun = True

                    # Check if name/symbol matches any trending term
                    is_trending = any(
                        term in name.lower() or term in symbol.lower()
        ...
        # Sort by Pump.fun first then newest
        gems.sort(key=lambda x: (not x["is_pumpfun"], -x["created_at"]))
        gems = gems[:8]

        msg = "*💎 Newest Trending Solana Memecoins*\n"
        msg += "━━━━━━━━━━━━━━━\n\n"
        ...
        for i, gem in enumerate(gems):
            ...
            trending_badge = "🔥 " if gem["is_trending"] else ""
            pump_badge = "🎯 " if gem["is_pumpfun"] else ""
            lp_badge = "🔒" if gem["lp_locked"] else "🔥LP" if gem["lp_burned"] else ""
            ...
            vol1h = gem["volume_1h"]
            vol1h_str = f"${vol1h/1_000_000:.2f}M" if vol1h >= 1_000_000 else f"${vol1h/1_000:.0f}K"

            liq = gem["liquidity"]
            liq_str = f"${liq/1_000_000:.2f}M" if liq >= 1_000_000 else f"${liq/1_000:.0f}K"
            ...
            msg += f"*{i+1}. {pump_badge}{trending_badge}{gem['name']} (${gem['symbol']})*  {lp_badge}\n"
            msg += f"`{gem['address']}`\n"
        ...
        msg += "━━━━━━━━━━━━━━━\n"
        msg += "🎯 = Pump.fun token\n"
        msg += "🔥 = CoinGecko trending\n"
        msg += "🔒 = LP locked | 🔥LP = LP burned\n"
        msg += "⚠️ _DYOR. Not financial advice._\n"
